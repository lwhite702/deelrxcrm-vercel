...
export default function AdminClient() {
  ...
  const [inactivityPolicies, setInactivityPolicies] = useState<
    InactivityPolicy[]
  >([]);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState<string | null>(null);
  const [selectedView, setSelectedView] = useState<
    "dashboard" | "purge" | "inactivity" | "monitoring"
  >("dashboard");
  ...
  const fetchPurgeOperations = async () => {
    try {
      ...
      setError(
        err instanceof Error ? err.message : "Failed to load purge operations"
      );
    } finally {
      setLoading(false);
    }
  ...
  const handleCreatePurgeOperation = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch("/api/admin/purge", {
        ...
        body: JSON.stringify({
          ...purgeForm,
          scheduledFor: purgeForm.scheduledFor
            ? new Date(purgeForm.scheduledFor).toISOString()
            : undefined,
        }),
      });
      ...
      setError(
        err instanceof Error ? err.message : "Failed to create purge operation"
      );
    }
  };
    ...
    try {
      ...
      setError(
        err instanceof Error
          ? err.message
          : "Failed to create inactivity policy"
      );
    }
  };
        ...
        <>
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
            <div className="urban-card">
              ...
              <p className="text-sm text-gray-400">
                {
                  purgeOperations.filter((op) => op.status === "executing")
                    .length
                }{" "}
                active
              </p>
          ...
          <div className="urban-card">
            ...
            <div className="space-y-3">
              {[
                ...purgeOperations,
                ...inactivityPolicies.map((p) => ({ ...p, type: "policy" })),
              ]
                .sort(
                  (a, b) =>
                  ...
                  >
                    ...
                    <div className="flex-1">
                      <p className="text-white">
                        {item.type === "policy"
                          ? `Inactivity policy "${item.name}" ${
                              item.isActive ? "activated" : "created"
                            }`
                          : `Purge operation for ${item.operationType.replace(
                              "_",
                              " "
                            )} ${item.status}`}
                      </p>
                ...
                <tbody>
                  {purgeOperations.map((operation) => (
                    <tr key={operation.id} className="border-b border-gray-800">
                      ...
                      <td
                        className={`py-3 font-medium ${getStatusColor(
                          operation.status
                        )}`}
                      >
                      ...
                      <td className="py-3 text-gray-300">
                        {operation.teamId.slice(0, 8)}...
                      </td>
                      <td className="py-3 text-gray-300">
                        {operation.retentionDays
                          ? `${operation.retentionDays} days`
                          : "N/A"}
                      </td>
                      <td className="py-3 text-gray-300">
                    ...
                    <div>
                      ...
                      <div className="flex gap-1">
                        {policy.actions.suspend && (
                          <span className="bg-yellow-600 text-white px-1 py-0.5 rounded text-xs">
                            Suspend
                          </span>
                        )}
            ...
            {inactivityPolicies.length === 0 && (
              <div className="text-center py-12">
                ...
                <p className="text-gray-400 mb-4">
                  No inactivity policies configured
                </p>
              </div>
            )}
          </div>
                ...
                <div className="flex items-center gap-2">
                  <input
                    ...
                    onChange={(e) =>
                      setPolicyForm((prev) => ({
                        ...prev,
                        suspendEnabled: e.target.checked,
                      }))
                    }
                    className="rounded bg-gray-800 border-gray-600"
                  />
                  <label htmlFor="suspendEnabled" className="text-gray-300">
                ...
                <div className="flex items-center gap-2">
                  <input
                    ...
                    onChange={(e) =>
                      setPolicyForm((prev) => ({
                        ...prev,
                        purgeEnabled: e.target.checked,
                      }))
                    }
                    className="rounded bg-gray-800 border-gray-600"
                  />
                  <label htmlFor="purgeEnabled" className="text-gray-300">
                    Enable account purge
                  </label>
                </div>
              </div>
