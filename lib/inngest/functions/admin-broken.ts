...
// TODO: Fix schema mismatches - disabled for now
export const adminPurgeExecute = inngest.createFunction(
  ...
  async ({ event, step }) => {
    ...
    return { message: "Admin purge function temporarily disabled" };
  }
);
...
export const adminInactivityCheck = inngest.createFunction(
  ...
  async ({ event, step }) => {
    // Temporarily disabled due to schema mismatches
    return { message: "Admin inactivity check function temporarily disabled" };
  }
);
...
export const adminRetentionEnforce = inngest.createFunction(
  ...
  async ({ event, step }) => {
    // Temporarily disabled due to schema mismatches
    return { message: "Admin retention enforce function temporarily disabled" };
  }
);
    ...
    try {
      ...
      const purgeScope = operation.purgeScope as any;
      if (purgeScope.criteria?.type === "inactive-teams") {
        purgedCount = await step.run("purge-inactive-teams", async () => {
          const daysInactive = purgeScope.criteria.daysInactive;
      ...
      await step.run("send-completion-notification", async () => {
        ...
        console.log(
          `Purge operation ${operationId} completed. ${purgedCount} records purged.`
        );
      });
...
export const adminInactivityScan = inngest.createFunction(
  ...
  async ({ event, step }) => {
    ...
    for (const policy of policies) {
      const policyResult = await step.run(
        `scan-policy-${policy.id}`,
        async () => {
          const cutoffDate = new Date(
            Date.now() - policy.inactiveDays * 24 * 60 * 60 * 1000
          );
          ...
          for (const entity of inactiveEntities) {
            ...
            if (existingTracker.length === 0) {
              ...
              console.log(
                `New inactivity detected for team ${entity.teamName} under policy ${policy.name}`
              );
            } else {
              // Check if we need to take action based on policy
              const tracker = existingTracker[0];
              const daysSinceDetection = Math.floor(
                (Date.now() - tracker.detectedAt.getTime()) /
                  (24 * 60 * 60 * 1000)
              );

              if (
                !tracker.actionTaken &&
                daysSinceDetection >= policy.gracePeriodDays
              ) {
                ...
                if (policy.actionType === "suspend") {
                  ...
                  console.log(
                    `Suspended team ${entity.teamName} due to inactivity`
                  );
                } else if (policy.actionType === "delete") {
                  // TODO: Implement team deletion
                  console.log(
                    `Would delete team ${entity.teamName} due to inactivity`
                  );
                }
    ...
    return {
      ...
      totalInactiveEntities: scanResults.reduce(
        (sum, result) => sum + result.inactiveCount,
        0
      ),
      results: scanResults,
    };
  ...
  async ({ event, step }) => {
    ...
    const activityHealth = await step.run("check-activity-levels", async () => {
      const recentActivity = await db
        ...
        .from(activityEvents)
        .where(
          gte(activityEvents.timestamp, new Date(Date.now() - 60 * 60 * 1000))
        ); // Last hour
    ...
    const operationsHealth = await step.run(
    ...
    if (unhealthyComponents.length > 0 || warningComponents.length > 0) {
      await step.run("send-health-alert", async () => {
        ...
        console.log("System health alert:", {
          unhealthyComponents,
          warningComponents,
        });
      });
