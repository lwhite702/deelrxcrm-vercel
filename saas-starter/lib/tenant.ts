...
import { tenants, tenantMembers, userSettings, type Tenant, type TenantMemberMT, type UserSettings } from './db/schema';
...
export async function getPersonalTenant(userId: number) {
  ...
  const rows = await db.select().from(tenants).where(eq(tenants.id, settings.personalTenantId));
...
export async function ensurePersonalTenant(userId: number) {
  ...
  await db
    ...
    .onConflictDoUpdate({ target: userSettings.userId, set: { personalTenantId: tenant.id } });
...
export async function listUserTenants(userId: number) {
  const rows = await db
    .select({ id: tenants.id, name: tenants.name, slug: tenants.slug, personal: tenants.personal })
    .from(tenantMembers)
...
export async function getActiveTenant() {
  ...
  if (cookieVal) {
    ...
    if (membership) {
      const rows = await db.select().from(tenants).where(eq(tenants.id, cookieVal));
      return rows[0] as Tenant | undefined;
    }
  }
